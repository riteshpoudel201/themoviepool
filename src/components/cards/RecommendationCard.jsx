...
const RecommendationCard = ({
  ...
  return (
    <Link to={`/show/${showType}/${show?.id}`} className="snap-center">
      <div className="snap-center relative w-full h-[100px] lg:h-[80px] rounded-lg shadow-lg overflow-hidden">
        {/* Movie Poster */}
        <img
        ...
        <div className="w-full h-full py-2 inset-0 bg-black/60 flex flex-col justify-between p-4 text-white relative z-[10]">
          {/* Movie Title */}
          <h3 className="text-lg text-white font-bold mb-2">{show?.title}</h3>
         
          {/* Watch Trailer Button */}
          <div className="w-full flex justify-between">
            <span className="text-gray-400">{formatYear(show?.release_date)}</span>
